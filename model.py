...
class PredictModel(tf.keras.Model):
    ...
    def call(self,x1,adjoin_matrix1,mask1,x2,adjoin_matrix2,mask2,t1,t2, training=False):
        x1 = self.encoder(x1,training=False,mask=mask1,adjoin_matrix=adjoin_matrix1)
        x1 = x1[:,0,:]
        x2 = self.encoder(x2,training=False,mask=mask2,adjoin_matrix=adjoin_matrix2)
        x2 = x2[:,0,:]
        x = tf.concat([x1,x2], axis=1)
        t2 = self.t2_processing_network(t2)
        x = tf.concat([x, t1, t2], axis=1)
        x = self.fc1(x)
